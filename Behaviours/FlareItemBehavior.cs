...
{
    internal class FlareItemBehavior : GrabbableObject
    {
        ...
        private NetworkVariable<FlareState> currentFlareState = new NetworkVariable<FlareState>(FlareState.NEW);
        ...
        public void Awake()
        {
            ...
            Plugin.mls.LogInfo("Item initialized in state: " + currentFlareState.Value);
        }
        ...
        public override void ItemActivate(bool used, bool buttonDown = true)
        {
            ...
            if (buttonDown)
            {
                switch (currentFlareState.Value)
                {
                    case FlareState.NEW:
        ...
        {
            base.EquipItem();
            if(currentFlareState.Value == FlareState.BURNING)
            {
        ...
        public void ChangeFlareStateServerRpc(FlareState newState)
        {
            currentFlareState.Value = newState;
        }

        public override void OnNetworkSpawn() 
        {
            base.OnNetworkSpawn();
            currentFlareState.OnValueChanged += OnFlareStateChanged;

            if(IsServer)
            {
                currentFlareState.Value = FlareState.NEW;
            }

            ResetFlareComponents();
        }

        private void ResetFlareComponents()
        {
            fireSource.enabled = false;
            smokeEffect.Stop();
            audioSource.Stop();
        ...
        private void OnFlareStateChanged(FlareState oldState, FlareState newState)
        {
            Plugin.mls.LogInfo("Changing state from " + oldState + " to " + newState);
            switch (newState)
            {
                case FlareState.BURNING:
        ...
        private void ChangeState(FlareState newState)
        {
            ...
            { 
                ChangeFlareStateServerRpc(newState); 
            }
        }
    }
}